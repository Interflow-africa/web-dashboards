...
import { settingsAPI, artistAPI, orgAPI, authAPI } from "@/services/index";
...
const SettingRow = ({
  ...
  <div
    className={`border rounded-xl transition-colors ${expanded ? "border-[#8D5D1D]/40 bg-[#FDFAF5]" : "border-gray-100 bg-white"}`}
  >
...
const SettingsPage = () => {
  ...
  /* Routes profile PATCH to the correct endpoint based on user role */
  const updateProfile = (data) =>
    role === 'organization' ? orgAPI.updateProfile(data) : artistAPI.updateProfile(data);
  ...
  const saveContactInfo = async () => {
    try {
      await updateProfile({
        phone_number: portContact.phone_number,
  ...
  const saveEmailAddr = async () => {
    try {
      ...
      toast.success(
        "Email update requested. Our team will confirm the change.",
      );
      close();
    } catch (err) {
  ...
  const handleCloseAccount = async () => {
    ...
    if (!closePassword) {
      toast.error("Password is required to close account");
      return;
    }
    try {
      ...
      toast.error(
        getApiError(
          err,
          "Failed to close account. Please check your password.",
        ),
      );
    }
  };
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field
                    ...
                    onChange={(e) =>
                      setPortName((p) => ({ ...p, firstName: e.target.value }))
                    }
                  />
                  <Field
                    label="Last name"
                    ...
                    onChange={(e) =>
                      setPortName((p) => ({ ...p, lastName: e.target.value }))
                    }
                  />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field
                    ...
                    onChange={(e) =>
                      setPortName((p) => ({ ...p, location: e.target.value }))
                    }
                  ...
                  <SelectField
                    ...
                    onChange={(e) =>
                      setPortName((p) => ({ ...p, pronouns: e.target.value }))
                    }
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field
                    label="Phone number"
                    type="tel"
                    value={portContact.phone_number}
                    onChange={(e) =>
                      setPortContact((p) => ({
                        ...p,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <Field
                    ...
                    onChange={(e) =>
                      setPortContact((p) => ({ ...p, city: e.target.value }))
                    }
                  />
                  <Field
                    label="Country"
                    ...
                    onChange={(e) =>
                      setPortContact((p) => ({ ...p, state: e.target.value }))
                    }
                  />
                  {/* <Field
                    label="Zip"
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Control where and what kind of notifications you get
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Connect a domain and update your Stagetime URL.
              </p>
            </div>
          </div>
            ...
            >
              <div className="flex flex-col gap-4">
                <p className="text-xs text-gray-500 leading-relaxed">
                  Your URL must contain 3–100 letters or numbers. Do not use
                  spaces, symbols, or special characters (e.g. !, $, @). Do not
                  put a web address in this field.
                </p>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Control your Interflow account
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            >
              <div className="flex flex-col gap-4">
                ...
                />
                <p className="text-xs text-gray-500 leading-relaxed">
                  If you'd like to update your email address, enter your new
                  email address above. Our team will reach out once we've
            ...
            >
              <div className="flex flex-col gap-4">
                ...
                <p className="text-xs text-gray-400 leading-relaxed">
                  By closing your account, you will permanently lose access to
                  your Interflow profile, portfolio, connections, and all
                  associated data. This action cannot be undone. Please ensure
                  you have saved any information you need before proceeding.
                </p>

                <div className="flex flex-col">
                  <label className="text-[12px] text-[#888] mb-1">
                    Message (optional)
                  </label>
                  <textarea
                    rows={4}
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = "#8D5D1D";
                    }}
                  >
                    Close my account
                  </button>
